...
def get_all_data_loaders(conf):
    ...
    loader=Getdataloader(conf['data_root'],batch_size,num_workers)
    train_loader_a=loader.train_dataloader_a()
    val_loader_a=loader.val_dataloader_a()
    train_loader_b=loader.train_dataloader_b()
    val_loader_b=loader.val_dataloader_b()
...
def vgg_preprocess(batch):
    ...
    batch = torch.cat((batch, batch, batch), dim = 1) # convert RGB to BGR
    batch = (batch/6 + 1) * 255 * 0.5 # [-1, 1] -> [0, 255]
